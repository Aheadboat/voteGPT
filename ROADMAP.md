...
## F1 — Development and Test Foundation [IN PROGRESS — REFACTOR]
...
- **GitHub authorization:** User approved a public `Aheadboat/voteGPT` repository on 2026-07-13. GitHub CLI is installed and authenticated as `Aheadboat`; the local repository has no remote and the named GitHub repository does not yet exist. Creation, initial push, and live Actions verification remain after local verification and Gate A.
...
## F2 — Identity and Public Shell [TODO]

- **Outcome:** Public profiles remain browseable while personalized pages require email-link or Google sign-in.
- **Dependencies:** F1.
- **Tests first:** Anonymous public access, protected-dashboard redirect, email-link session, Google session, and logout revocation.
- **Done:** Auth.js and minimal PostgreSQL/Drizzle persistence pass session and authorization tests; local/test email uses a fake; credentials remain server-side; account-deletion foundation exists.
