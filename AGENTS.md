...
- **Initial scope and base:** The user's initial 2026-07-16 authorization covered the ordered feature set F4, F5, F6, F7, and F8 on `codex/autonomous-f4-f8-integration`, forked from actual `main` at `d5978ba830f0ee715c9162afba8963139c0fb707`. It also covered the prerequisite as a read-only, no-commitment G1 evaluation plus a public-source fallback; there was no paid vendor commitment and no binding vendor account, terms, quote, spend, or credential.
- **Review-correction authority:** The original autonomous window ended when F5's closeout merge placed both items `DONE` on the integration branch. On 2026-07-17 the user explicitly authorized F4 and F5 review corrections, approved the identified fixes and the hybrid district-validation direction, required hardcoded policy values and magic numbers to be avoided where authoritative data or named policy constants exist, and required one fresh implementer subagent per bounded task. This authority does not activate F6, G1, F7, or F8; those items remain TODO/inactive and require fresh explicit human direction.
...
- **Correction admission:** The coordinator's read-only dependency/interface/admission audit records `CONDITIONAL`: F4 exclusively owns the saved-residence schema/revision and trust-boundary correction surfaces and closes first; F5 may use disjoint federal-only lanes, then integrates the F4 correction closeout before F5 final integration and the shared-surface handoff. At most two roadmap items may be active. The correction worktrees are based on integration commit `d2c856a206cd4a7b8cf71958da0465fe414dbac6`.
- **Human gates:** The approved direction is not a substitute for the written design and tests-first task graph. The coordinator presents those artifacts at explicit Human Gate A and pauses for user review before RED or production work. After task-level independent reviews, fresh focused/full verification, hosted CI, mergeability, and whole-feature adversarial review, the coordinator presents explicit Human Gate B and pauses before any cumulative merge to actual `main`.
- **Quality, role, and dispatch boundaries:** TDD, privacy, security, accessibility, civic/editorial integrity, provider-rights checks, equal treatment, source/freshness evidence, feature-lead isolation, and conflict recovery remain binding. The coordinator remains coordinator-only and does not implement feature production code. Use one fresh implementer subagent per bounded task, followed by a separate read-only task reviewer; do not give one agent the whole correction set. When blocked, record the blocker, continue every independent admitted lane, and defer the exact decision to final human review.
- **Final handoff:** F4 correction feature/closeout work precedes F5's final integration and closeout. A cumulative PR to actual `main` carries all corrected evidence and blockers for final human review and must not be merged autonomously. Only the user's later explicit Human Gate B approval may integrate completed work into actual `main`; F6, G1, F7, and F8 remain inactive.
