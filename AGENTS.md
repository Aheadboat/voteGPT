...
- **Scope and base:** The user's 2026-07-16 standing authorization covers the ordered feature set F4, F5, F6, F7, and F8 on `codex/autonomous-f4-f8-integration`, forked from actual `main` at `d5978ba830f0ee715c9162afba8963139c0fb707`. It also covers the prerequisite as a read-only, no-commitment G1 evaluation plus a public-source fallback; there is no paid vendor commitment and no binding vendor account, terms, quote, spend, or credential.
...
- **Final handoff:** The handoff occurs when all authorized items close, when the autonomous window ends or remaining work is human-blocked, whichever comes first. The coordinator opens or updates a cumulative PR from `codex/autonomous-f4-f8-integration` to actual `main` with completed evidence, partial state, and blockers for final human review and must not merge it. Only the user's later approval may integrate the batch into actual `main`.
