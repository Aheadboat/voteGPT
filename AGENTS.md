...
- **Onward authority superseded:** Later on 2026-07-16 the user ended onward autonomous work after F5. The earlier standing authority remains sufficient only to complete F5 through feature merge, post-merge verification, and closeout; it no longer authorizes activating F6, G1, F7, or F8. Those items remain TODO/inactive and require fresh explicit human direction.
...
- **Admission:** The initial authorization placed work in a queue rather than activating it. The later user instruction ended that queue after F5; F6, G1, F7, and F8 remain inactive. At most two roadmap items may be active. Any future activation requires fresh explicit human direction plus the normal read-only dependency/interface/admission audit, ownership and merge-order record, and dependency-complete base. Coupled work remains sequential.
- **Delegated gates:** The user delegated interim approvals so the coordinator may satisfy delegated Gate A after a complete design and tests-first plan receive independent review with no unresolved Critical or Important finding. The coordinator may satisfy delegated Gate B only after fresh focused and full verification, independent whole-feature review, hosted CI, mergeability, and the complete evidence packet. Within this scope the coordinator does not pause for additional permission. Records must identify the user's standing authorization and the coordinator's gate decision; they must not claim the user reviewed an unseen artifact.
- **Quality and role boundaries:** TDD, privacy, security, accessibility, civic/editorial integrity, provider-rights checks, equal treatment, source/freshness evidence, feature-lead isolation, and conflict recovery remain binding. The coordinator remains coordinator-only and does not implement feature production code. When a human-only decision is needed, record the blocker, continue every independent admitted lane, and defer the exact decision to final human review; do not prompt for permission during the batch.
- **Final handoff:** The autonomous window ends when F5's closeout merge places `DONE` on `codex/autonomous-f4-f8-integration`. The coordinator then stops, leaving F6, G1, F7, and F8 inactive for fresh explicit human direction. Any cumulative PR to actual `main` carries completed evidence and blockers for final human review and must not be merged autonomously. Only the user's later approval may integrate completed work into actual `main`.
